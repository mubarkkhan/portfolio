...
export default function ChatBox() {
  ...
  const handleCreateRoom = (e) => {
    e.preventDefault();
    if (user.name.trim() && user.email.trim() && roomName.trim()) {
    ...
    <div className="fixed bottom-5 right-5 z-50">
      {open ? (
        <div className="w-80 h-[400px] bg-white rounded-2xl shadow-xl border border-gray-200 flex flex-col overflow-hidden">
          ...
          {!isChatStarted ? (
            <form onSubmit={handleCreateRoom} className="p-4 space-y-3 bg-gray-50">
              <p className="text-xs text-gray-500 text-center">
                Enter your name & email to create a private room. Share the link with friends to chat together!
              </p>
              <input
  type="text"
...
/>
            ...
            <>
              {/* Optional: show messages here if you also implement inline chat */}
            </>
          )}
        </div>
      ) : (
